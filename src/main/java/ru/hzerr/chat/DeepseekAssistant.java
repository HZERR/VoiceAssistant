...
public class DeepseekAssistant implements IAssistant {
    ...
    private String getInstructionsV2() {
        return """
            ...
            JSON не нужно форматировать (добавлять пробелы или переводы строки)

            📦 Примеры:
            Только голосовой ответ:{"speak":"<speak>Добрый день, Вадим.</speak>","commands":[]}
            Только команда: {"speak":"","commands":[{"command":"getCpuInformation","args":[]}]}
            Ответ + команда:{"speak":"<speak>Запрашиваю информацию...</speak>","commands":[{"command":"getCpuInformation","args":[]}]}
            ---
            ⚠️ Поведение:
            - Старайся не сокращать слова. Например: гб - гигабайт. Т.к твой текст внутри <speak> синтезируется в речь яндексом и сокращения система не понимает.
